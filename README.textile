...
** *autoload:* (true) Automatically load the dataset upon initialization, favoring offline data if enabled and available in the local storage. If offline load fails, defaults to loading live data.
** *offline:* (_false_) Serializes and stores the collection for offline use whenever the page is unloaded.
...
_Returns: boolean (for success/failure)_
